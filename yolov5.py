...
class YoloV5:
    def __init__(self, yolov5_yaml_path='config/yolov5port.yaml'):
        '''初始化'''
        # 载入配置文件
    ...
    def plot_one_box(self, x, img, color=None, label=None, line_thickness=None):
        ...
        global c3
        c3 = int((int(x[0]) + int(x[2])) / 2), int((int(x[1]) + int(x[3])) / 2)  # center point
        # distance = depth_frame.get_distance(c3[0], c3[1])
        # print(c3)
        c4 = (label,)
        temp = c4 + c3
        global result
        ...
        cv2.circle(img, c3, 4, (255, 255, 255), 5)  #
        cv2.putText(img, str(c3), (c3[0] + 0, c3[1] + 0), 0, tl / 3, [225, 255, 255], thickness=2, lineType=cv2.LINE_AA)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if key == ord('q'):
            str = '\n'
            f = open("out.txt", "w")
            f.write(str.join('%s' % id for id in result_log))
            f.close()
            break
